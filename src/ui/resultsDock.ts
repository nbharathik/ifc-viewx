...
export interface DocketRow {
  ...
  title: string;
  detail?: string;
  /** What this row belongs with: a rule, a class pair, a specification. */
  group?: string;
  ids: number[];
  point?: [number, number, number];
  /** Who owns it, once a reviewer has said. */
  assignee?: string;
  status?: "open" | "accepted" | "rejected";
}

export interface DocketSet {
  /** Stable per producer, so a re-run replaces rather than stacks. */
  id: string;
  producer: string;
  title: string;
  summary: string;
...
/** Publish a result set. The same id replaces the previous run. */
