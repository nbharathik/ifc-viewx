...
??? info "Element Explorer: A spreadsheet over every element and property in the model"
...
??? info "Model Finder: Search the whole model in plain words, then act on what comes back"
...
??? info "Model Health: Catch model identity and geometry problems before review"
...
??? info "Quantity Takeoff: Volumes, areas and counts rolled up by class and storey"
...
??? info "Storey Navigator: Walk the building one level at a time"
...
??? info "Assistant Key Vault: Keep the provider key off the browser"
...
??? info "IfcOpenShell Converter: Exact solids and instant reopens"
...
??? info "MCP Bridge: Let Claude and other AI clients drive this viewer"
...
??? info "Native Python: Full IfcOpenShell scripting with no runtime download"
...
??? info "Element Schedules: Tabular exports with resolved property columns"
...
??? info "IDS Validation: Check the model against a buildingSMART specification"
...
??? info "Model Checks: Structural QA over the whole file"
...
??? info "Model Edits: Rename, set properties and delete, with a measured diff"
...
??? info "Smart Filters: Rule based visibility that you can stack and undo"
